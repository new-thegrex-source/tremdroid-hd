...
static void CG_AlienBuilderText( char *text, playerState_t *ps )
{
  ...
  if( ps->stats[ STAT_CLASS ] == PCL_ALIEN_BUILDER0_UPG || ps->stats[ STAT_CLASS ] == PCL_ALIEN_BUILDER0_CRZ )
  {
    Q_strcat( text, MAX_TUTORIAL_TEXT,
...
/*
===============
LOL so for my super nuclear grnger
===============
*/
static void CG_AlienLevel0Text2( char *text, playerState_t *ps )
{
  Q_strcat( text, MAX_TUTORIAL_TEXT,
      "Stay near a human to expose him to radiation\n" );

  Q_strcat( text, MAX_TUTORIAL_TEXT,
      "Touch a human to infect him\n" );
  ...
  if( !cg.intermissionStarted && !cg.demoPlayback )
  {
    ...
    else if( ps->stats[ STAT_HEALTH ] > 0 )
    {
      switch( ps->stats[ STAT_CLASS ] )
      {
        ...
        case PCL_ALIEN_LEVEL0_NUC:
          CG_AlienLevel0Text2( text, ps );
          break;
