...
static qboolean CG_ParseAnimationFile( const char *filename, clientInfo_t *ci )
{
  ...
  while( 1 )
  {
	...
	else if( !Q_stricmp( token, "lang" ) )
    {
      ...
      if( token[ 0 ] == 'f' || token[ 0 ] == 'EN' )
        ci->lang = en_EN;
      else if( token[ 0 ] == 'n' || token[ 0 ] == 'P' )
        ci->lang = pl_PL;
      else
        ci->lang = ru_RU;
...
static void CG_PlayerUpgrades( centity_t *cent, refEntity_t *torso )
{
  ...
  if( held & ( 1 << UP_AMMOPACK ) )
  {
    memset( &ammopack, 0, sizeof( ammopack ) );
    VectorCopy( torso->lightingOrigin, ammopack.lightingOrigin );
    ammopack.shadowPlane = torso->shadowPlane;
    ammopack.renderfx = torso->renderfx;

    ammopack.hModel = cgs.media.ammopackModel;

    //identity matrix
    AxisCopy( axisDefault, ammopack.axis );

    //FIXME: change to tag_back when it exists
    CG_PositionRotatedEntityOnTag( &ammopack, torso, torso->hModel, "tag_head" );

    trap_R_AddRefEntityToScene( &ammopack );
  }
