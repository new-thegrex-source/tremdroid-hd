...
void slowBlobFire2( gentity_t *ent )
{
  G_CombatStats_Fire( ent, CSW_ABUILDER_ALT, ABUILDER_BLOB_DMG_UPG );
  fire_slowBlob2( ent, muzzle, forward );
}
    ...
    else
    {
        ...
        for( i = 0; i < num; i++ )
        {
            ...
            if( enemy->flags & FL_NOTARGET )
                continue;
            if( enemy->client && (enemy->client->ps.stats[ STAT_TEAM ] != ent->client->ps.stats[ STAT_TEAM ] ) )
            {
...
===============
*/
qboolean CheckVenomAttack3( gentity_t *ent )
{
  trace_t   tr;
  gentity_t *traceEnt;
  int       damage = LEVEL0_BITE_DMG_NUC;
  ...
  G_WideTrace( &tr, ent, LEVEL0_NUC_BITE_RANGE, LEVEL0_NUC_BITE_WIDTH,LEVEL0_NUC_BITE_WIDTH, &traceEnt );
...
//OriginPart:
  ...
  */ 
  //well i disabled it, Fail TROLOLOLOLO (c) NewSource
  if( traceEnt->client )
  {
    if( traceEnt->client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
      return qfalse;
  ...
  // send blood impact
  WideBloodSpurt( ent, traceEnt, &tr );
  G_Damage( traceEnt, ent, ent, forward, tr.endpos, damage, DAMAGE_NO_KNOCKBACK, MOD_LEVEL0_NUC );
  ent->client->ps.weaponTime += LEVEL0_BITE_REPEAT_NUC;
  return qtrue;
}
...
//LOL FOR MY MEGA SHIT!!!@#!#@!!@!

void poisonCloud2( gentity_t *ent )
{
  int       entityList[ MAX_GENTITIES ];
  vec3_t    range = { LEVEL1_PCLOUD_RANGE, LEVEL1_PCLOUD_RANGE, LEVEL1_PCLOUD_RANGE };
  vec3_t    mins, maxs;
...
void bounceBallFire_level2( gentity_t *ent )
{
  ...
  m = fire_bounceBall2( ent, muzzle, forward,
                       WP_ALEVEL2, LEVEL2_BOUNCEBALL_DMG,
...
void bounceBallFire_level3( gentity_t *ent )
{
  gentity_t *m;

  m = fire_bounceBall2( ent, muzzle, forward,
                       WP_ALEVEL3_UPG, LEVEL3_BOUNCEBALL_DMG,
                       MOD_LEVEL3_BOUNCEBALL, LEVEL3_BOUNCEBALL_SPEED,
                       LEVEL3_BOUNCEBALL_RADIUS );
...
void FireWeapon( gentity_t *ent )
{
  ...
  switch( ent->s.weapon )
  {
    ...
    case WP_MGTURRET:
      ...
      break;
     case WP_MGTURRET3:
	  bulletFire( ent, MGTURRET3_SPREAD, MGTURRET3_DMG, MOD_MGTURRET3 ); 
      break; 	  
    case WP_ABUILD:
