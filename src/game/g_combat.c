...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
{
  ...
  if( self && self->client && BG_InventoryContainsUpgrade( UP_BATTPACK, self->client->ps.stats ) )
  {
	  G_RadiusDamage(self->client->ps.origin, self, 50, 50, self, MOD_TARGET_LASER);
  }
  ...
  {
	  trap_SendServerCommand( -1,
	       va( "print \"%s^7 ^5was electrocuted by %s^7 ^5battpack\n\"",
	       self->client->pers.netname, attacker->client->pers.netname));
	     G_LogPrintf("%s^7 ^5was electrocuted by %s^7 ^5battpack\n",
	       self->client->pers.netname, attacker->client->pers.netname );
  }
 ...
 //       return;
      ...
      if( ( mod == MOD_LEVEL4_TRAMPLE || mod == MOD_LEVEL4_CRUSH ) &&
          ...
          ( targ->client->ps.stats[ STAT_CLASS ] == PCL_ALIEN_BUILDER0 ||
            ...
            targ->client->ps.stats[ STAT_CLASS ] == PCL_ALIEN_BUILDER0_CRZ ) ) 
