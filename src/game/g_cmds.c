...
qboolean G_ClassIsAllowed( class_t class ) {
  ...
  if (g_minTeamSizePerBuilderA.integer > 0) {
    if (class == PCL_ALIEN_BUILDER0 || class == PCL_ALIEN_BUILDER0_UPG || PCL_ALIEN_BUILDER0_CRZ) {
      CalculateRanks();
...
//          newClass != PCL_ALIEN_BUILDER0_CRZ &&
    ...
    if( ent->client->pers.classSelection != PCL_NONE )
    {
      ...
      if( ent->client->sess.spectatorState == SPECTATOR_NOT &&
          ( currentClass == PCL_ALIEN_BUILDER0 ||
            ...
            currentClass == PCL_ALIEN_BUILDER0_CRZ ) &&
...
void Cmd_Build_f( gentity_t *ent )
{
  ...
  if( buildable != BA_NONE &&
      ( ent->client->ps.weapon < 32 && ( ( 1 << ent->client->ps.weapon ) & BG_Buildable( buildable )->buildWeapon ) ) &&
...
void Cmd_Share_f( gentity_t *ent )
{
  ...
  {
    trap_SendServerCommand( ent-g_entities, "print \"^5Share has been disabled.\n\"" );
    return;
  }
  ...
  if( trap_Argc( ) < 3 )
  {
    ADMP( va( "^5usage: %s [name|slot#] [amount]\n", cmd ) );
  }
  ...
  if( creds <= 0 )
  {
    ADMP( va( "%s: %s ^5cannot receive any more %s\n", cmd,
          level.clients[ clientNum ].pers.netname,
  ...
  ADMP( va( "%s: ^5transferred^7 %d %s ^5to^7 %s^7.\n\"", cmd, creds,
        ( team == TEAM_HUMANS ) ? "credits" : "evos",
        level.clients[ clientNum ].pers.netname ) );
  ...
  G_LogPrintf( "^5Share:^7 %i %i %i %d: %s^7 ^5transferred^7 %d%s ^5to^7 %s^7\n",
...
void Cmd_Donate_f( gentity_t *ent )
{
  ...
  if( !g_allowShare.integer )
  {
    ADMP( "^5Donate has been disabled.\n" );
    return;
  }
