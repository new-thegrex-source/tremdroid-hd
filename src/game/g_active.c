...
/*
  if( client->ps.weapon == WP_ALEVEL6 &&
      client->lastWarpTime + LEVEL6_WARP_REGEN_DELAY <= level.time )
  {
    client->ps.stats[ STAT_MISC ] += msec * LEVEL6_WARP_REGEN_RATE;

    if( client->ps.stats[ STAT_MISC ] > LEVEL6_WARP_TIME )
    {
      client->ps.stats[ STAT_MISC ] = LEVEL6_WARP_TIME;
    }
  }
 */ 
  ...
  if( client->pers.paused )
     client->ps.speed = 0;
  else if( client->ps.pm_type == PM_NOCLIP )
    client->ps.speed = client->pers.flySpeed;
  else
